...
# Marco Baldo, CULS, 05.05.2022


# Loop to see into the transitional period but interactively
...
dataroot <- ("C:/iLand/2022/20220604_final_test/DB/")                        # Root for the selection of the data
...
cases <-c("B0_Tby0_PRby0","B0_Tby5_PRby0","B1_Tby0_PRby0", "B1_Tby5_PRby0", "Btot_Tby0_PRby0", "Btot_Tby5_PRby0")
...
for (i in (1:length(cases)))  {                                        # We read in the files in the loop. The "i" is for the x-> 1:i 
  ...
  # FOR MAKE THE na = 0 !!!! "damage$wind[which(is.na(damage$wind)==TRUE)] <-0"
  head(damage)
  
  # plot(damage$barkbeetle,typ='l', ylim=c(0,1650000))
  # points(damage$wind)

#------ The real code for the wind table  ------------#

dataroot<-"C:/iLand/wind/random_wind_event_new_script/"

# a<-read.table(paste0(dataroot,winds[i],"_", scens[j],".txt"), header=T)   # read the text file on my wind text
...
# events<-which(a$year==20|a$year==50|a$year==80|a$year==110|a$year==140|a$year==170)  # which year I want and in specific years
...
# write.table(a,paste0(dataroot,winds[i],"_",scens[j],"_upto2200.txt"), col.names = T, quote=F,row.names=F, sep=" ")  # write the table
